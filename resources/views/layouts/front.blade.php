            ...
            <ul class="navbar-nav ml-auto">
              ...
              <li class="nav-item">
                @if (empty(Auth::user()->image_path))
                  <a class="nav-link" href="{{ action('Admin\UserController@show', ['id' => Auth::user()->id]) }}"><img src="{{ asset('storage/image/neko.jpg') }}" style="width:25px;height:25px" class="rounded-circle">{{ Auth::user()->name }}</a>
                @else
                  <a class="nav-link" href="{{ action('Admin\UserController@show', ['id' => Auth::user()->id]) }}"><img src="{{ asset('storage/image/' . Auth::user()->image_path) }}" style="width:25px;height:25px" class="rounded-circle">{{ Auth::user()->name }}</a>
                @endif
              </li>
              <li>
