    ...
    <div class="order-info col-md-11 mx-auto">
      <div clas="row">
        <table class="table table-bordered bg-white">
          ...
          <tbody>
            ...
            <td width="80%">
              @if (empty($order->client()->name))
                ユーザは不在です。
              @else
              <a href="{{ action('Admin\UserController@show', ['id' => $order->client()->id]) }}">{{ $order->client()->name }}</a></td>
              @endif
            </td>
          </tbody>
          <tbody>
          ...
          <form action="{{ action('Admin\OrderController@evaluate_client') }}" method="post">
            @if (count($errors) > 0)
            <ul>
          ...
          <tbody>
              ...
              <td width="80%">
                  <div class="evaluation text-left">
                    <input id ="paw1" type="radio" name="client_eval_point" value="5">
                    <label for="paw1"><i class="fa fa-paw mr-1"></i> </label>
                    <input id ="paw2" type="radio" name="client_eval_point" value="4">
                    <label for="paw2"><i class="fa fa-paw mr-1"></i></label>
                    <input id ="paw3" type="radio" name="client_eval_point" value="3">
                    <label for="paw3"><i class="fa fa-paw mr-1"></i></label>
                    <input id ="paw4" type="radio" name="client_eval_point" value="2">
                    <label for="paw4"><i class="fa fa-paw mr-1"></i></label>
                    <input id ="paw5" type="radio" name="client_eval_point" value="1">
